...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	@Nullable//bean的作用范围，对应bean的scope属性
	private String scope = SCOPE_DEFAULT;

	//是否是抽象，对应bean属性abstract
	private boolean abstractFlag = false;
	//是否懒加载，对应bean属性lazy-init
	private boolean lazyInit = false;
	//自动注入模式，对应bean属性autowride
	private int autowireMode = AUTOWIRE_NO;
	//依赖检查，Spring3,0后医用这个属性
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;

	@Nullable
	private String[] dependsOn;//用来标识一个bean的实例化依靠另一个bean，

	//设置为false，容器在查找自动装配对象时将不考虑该bean，但是其本身还是可以使用自动注入 来注入其他bean；
	private boolean autowireCandidate = true;

	//如果容器中存在多个相同类型的bean，设置为true会使用该bean
	...
	//是否以一种宽松模式解析构造函数，默认为true；如果为false，则在如下情况下
	/**
	 * interface ITest{}
	...
	/**
	 * 对应bean属性的 factory-bean，用法：
	 *<bean id="hahaha" class="com.service.HaImpl"></bean>
	  <bean id="teacher" factory-bean="student" factory-method="getPeople"></bean>
	 */
	@Nullable
